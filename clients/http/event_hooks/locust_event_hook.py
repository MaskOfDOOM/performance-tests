...
def locust_request_event_hook(request: Request) -> None:
    ...
    Сохраняет текущее время в 'request.extentions["start_time"]',
...
def locust_response_event_hook(environment: Environment):
    ...
    Использует 'request.extentions["start_time"]' для вычисления времени отклика.
    Извлекает route из 'request.extentions["route"]', если задан.
    Отправляет собранные метрики в 'enviroments.events.request', чтобы Locust мог агрегировать статистику.
